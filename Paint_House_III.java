...
public class Paint_House_III {
	...
	public int minCostM(int[] houses, int[][] cost, int m, int n, int target) {
       ...
       int ans = solveM(0,0,0,houses,cost,m,n,target,memo);
        // if ans is INT_MAX/2, i.e answer is not possible
        if(ans == Integer.MAX_VALUE/2) return -1;
        return ans;
       
    }
    int solveM(int idx, int last, int t, int[] houses, int[][] cost, int m, int n, int target,int memo[][][]){
        ...
        if(houses[idx] == 0){ // the house can be painted with any of colour 1 to n
            for(int j=0; j<n; j++){
                ans = Math.min(ans, cost[idx][j] + solveM(idx+1, j+1, last == j+1?t:t+1, houses, cost,m,n,target,memo));
            }
        }
            ...
            ans = Math.min(ans, solveM(idx+1, houses[idx], last == houses[idx]?t: t+1, houses, cost, m, n, target,memo));
        }
        memo[idx][last][t]=ans;
        return memo[idx][last][t];
    }
