...
public class Interleaving_String {
	...
	public boolean isInterleaveR(String s1, String s2, String s3) {
        ...
        if(s1.charAt(0)==s3.charAt(0)){
            op1=isInterleaveR(s1.substring(1),s2,s3.substring(1));
        }
        boolean op2=false;
        if(s2.charAt(0)==s3.charAt(0)){
            op2=isInterleaveR(s1,s2.substring(1),s3.substring(1));
        }
        
        return op1||op2;
    }
	//Memoization
	public boolean isInterleaveM(String s1, String s2, String s3) {
        int s1Len=s1.length();
        int s2Len=s2.length();
        int s3Len=s3.length();
